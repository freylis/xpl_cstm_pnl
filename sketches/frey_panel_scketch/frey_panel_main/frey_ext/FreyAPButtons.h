...
class FreyAPButtons {
    ...
    public:
        ...
        void readFullState(String fullState) {
            ...
            if (fullState[37] == '1') {analogWrite(pinVNAVEnabled, KD2_LIGHT);}
             else {analogWrite(pinVNAVEnabled, LOW);};

            if (fullState[40] == '1') {analogWrite(pinLNAVEnabled, KD2_LIGHT);}
             else {analogWrite(pinLNAVEnabled, LOW);};

            if (fullState[43] == '1') {analogWrite(pinVORLOCKEnabled, KD2_LIGHT);}
             else {analogWrite(pinVORLOCKEnabled, LOW);};

            if (fullState[46] == '1') {analogWrite(pinAPPROACHEnabled, KD2_LIGHT);}
             else {analogWrite(pinAPPROACHEnabled, LOW);};

            if (fullState[49] == '1') {analogWrite(pinCMDEnabled, KD2_LIGHT);}
             else {analogWrite(pinCMDEnabled, LOW);};

            if (fullState[52] == '1') {analogWrite(pinCWSEnabled, KD2_LIGHT);}
             else {analogWrite(pinCWSEnabled, LOW);};

            if (fullState[61] == '1') {analogWrite(pinLevelChangedEnabled, KD2_LIGHT);}
